...
function statusUpdate(b,a,c){$("#"+b.userid).length&&($("#"+b.userid).chatbox("option").messageSent(b.userid,b,a),$("#ta"+b.userid).prop("disabled",c))}function comman_chatbox_update(b,a){$("div#chat_room").length&&$("#chat_room").chatroom("option").messageSent(b,a)}function newStatus(b){$.each(b.message,function(a,c){null!=b.newuser&&c.userid==b.newuser&&(statusUpdate(c,"Online",!1),comman_chatbox_update(c,"Online"))})}
