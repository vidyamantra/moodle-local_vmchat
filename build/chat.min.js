...
idList=[];tabs.tabs("refresh");localStorage.clear();vmstorage={};localStorage.setItem("init","false");sessionStorage.removeItem("chatroom");sessionStorage.clear()}},function(){b("#stickybar").removeClass("minimize").addClass("maximize");b("#hide_bar input").removeClass("expand").addClass("close");b("#hide_bar input").prop("title","Close chat");localStorage.clear();io.init(dataobject)});uiFooterbarchatroomtab=(this.uiFooterbarchatroomtab=b("<div></div>")).addClass("vmchat_room_bt").prop("id","chatroom_bt").appendTo(a);
...
function display_error(b){$("<div id = 'dialog' title = 'VmChat Error:'></div>").prependTo("#stickybar");$("#dialog").html(b);$("#dialog").dialog()}function isIosDevices(){var b=navigator.userAgent;return/iPad/i.test(b)||/iPhone OS 3_1_2/i.test(b)||/iPhone OS 3_2_2/i.test(b)};var lang={online:"Online Users",whos:"Who's online",chatroom:"Chatroom",chatroom_header:"Common Chat",error:"Something bad happend. Sorry!",wserror:"Browser does not support WebSocket",sterror:"This browser doesn't support Web Storage standard"};
